 ...
 *  camera
 */
Camera camera(glm::vec3(0.0f, 0.0f, 13.0f));  /* NOLINT */
...
int main() {
    ...
    GLuint subroutineIndex = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "calculatePointLight");
    GLuint subroutineIndex2 = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "calculateNonAttenLight");

    Model backpack = Model(std::string(backpackPath));

    glm::vec3 lightPos(4.0f, 3.0f, 0.0f);
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        glm::mat4 model(1.0f);
        model = glm::rotate(model, glm::radians(15.0f) * currentTime, glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        SetUniform(program, "lightColor", lightColor);
        SetUniform(program, "viewerPos", camera.position);

        // first subroutine
        glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &subroutineIndex2);
