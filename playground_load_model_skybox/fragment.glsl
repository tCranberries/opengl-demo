...
//    result += calcSpotLight(spotLight, vNormal, vFragmentPosition, cameraPosition);
    ...
    // reflection
    float ratio = 1.0 / 1.58;
    vec3 tmpVec = normalize(vFragmentPosition - cameraPosition);
//    vec3 sampleVec = reflect(tmpVec, normalize(vNormal));
    vec3 sampleVec = refract(tmpVec, normalize(vNormal), ratio);
    result += texture(skybox, sampleVec).rgb;
