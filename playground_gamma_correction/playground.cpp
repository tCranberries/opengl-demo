 ...
 *  camera
 */
Camera camera(glm::vec3(1.0f, 1.0f, 4.0f));  /* NOLINT */
...
void processInput(GLFWwindow* _window) {
    ...
    if (glfwGetKey(_window, GLFW_KEY_SPACE) == GLFW_PRESS && !gammaEnabledKeyPressed) {
        gammaEnabled = !gammaEnabled;
        gammaEnabledKeyPressed = true;
    }
    if (glfwGetKey(_window, GLFW_KEY_SPACE) == GLFW_RELEASE) {
        gammaEnabledKeyPressed = false;
    }
}
...
int main() {
    ...
    GLuint textureWithGammaCorrection = loadTexture(floorTexPath, true);
    GLuint texture = loadTexture(floorTexPath, false);
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        SetUniform(program, "lightPositions", 4, lightPositions);
        SetUniform(program, "lightColors", 4, lightColors);
        SetUniform(program, "cameraPosition", camera.position);
        SetUniform(program, "gamma", gammaEnabled);

        glBindVertexArray(VAO);
        glBindTexture(GL_TEXTURE_2D, gammaEnabled ? textureWithGammaCorrection : texture);
    ...
    glDeleteTextures(1, &textureWithGammaCorrection);
